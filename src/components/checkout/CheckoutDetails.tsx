...
---\n
_Order Items:_\n
${cartItems?.map((item) => `- ${(item.product as Product).meta?.title} ${item.variant && '(' + ((item.variant as Variant).options[0] as VariantOption).label + ')'}: ${item.quantity}x${(item.product as Product).priceInNGN}`).join('\n')}
---\n
_Checkout Details:_\n${Object.entries(data)
      .map(([key, value]) => `- ${key.toUpperCase()} => ${value === '' ? 'NIL' : value}`)
      .join('\n')}`
